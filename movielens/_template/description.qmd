...
pagetitle: {{ ds.name }} Data Description
order: 0
echo: false
deps:
- stats.duckdb
...
```{python}
item_lognorm = sps.fit(sps.lognorm, item_stats['n_ratings'], {
    's': (1e-6, 1e3),
    'loc': (0, 100),
    'scale': (0.1, 1e3),
})
...
item_dfits = {
    n: sps.fit(dist, item_stats['n_ratings'], space)
    for n, (dist, space) in item_dists.items()
}
```


```{python}
xs = np.linspace(1, item_stats['n_ratings'].max(), 500)
plt.ecdf(item_stats['n_ratings'], color='black', label='Empirical')
for name, fit in item_dfits.items():
    dist, _space = item_dists[name]
    plt.plot(xs, dist.cdf(xs, *fit.params), linestyle='dashed', label=name)
plt.xscale('symlog')
