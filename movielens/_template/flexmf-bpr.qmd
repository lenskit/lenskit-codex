---
title: FlexMF BPR
pagetitle: {{ ds.name }} - FlexMF BPR
echo: false
model: flexmf-bpr
deps:
{%- for split in ds.splits %}
{%- for search in ds.searches %}
- sweeps/{{split}}/flexmf-bpr-{{search}}
{%- endfor %}
{%- endfor %}
---

This page analyzes the hyperparameter tuning results for the FlexMF scorer in
implicit-feedback mode with pairwise loss (Bayesian Personalized Ranking).
...
```{python}
runs = load_sweep_runs('flexmf-bpr')
iters = load_sweep_iters('flexmf-bpr')
result = load_sweep_result('flexmf-bpr')
...
```{python}
from codex.models.flexmf_bpr import SEARCH_SPACE
...
The model structure — particularly embedding size — is the hyperparameter that
most affects the model's fundamental logic, so let's look at performance as a
fufnction of it:
...
(
    pn.ggplot(runs)
    + pn.aes(x='config.item_bias', y='RBP')
    + pn.geom_point()
    + pn.xlab("lg(embedding size)")
)
```
