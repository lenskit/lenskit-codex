...
```{python}
with open('sweeps/random/BiasedMF-ALS/runs.json', 'rt') as jsf:
...
```{python}
cfg_rmse = runs.groupby(['params.features', 'params.reg'])['metrics.RMSE'].mean().reset_index()
...
```{python}
cfg_topn = runs.groupby(['params.features', 'params.reg'])[['metrics.RBP', 'metrics.NDCG']].mean().reset_index()
